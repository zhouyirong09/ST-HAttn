...
class ST_Attn(object):
    def __init__(self, input_dim=[64, 64, 2], batch_size=32, input_steps=10, output_steps=10, ext_dim=8,
        ...
        self.bn_training = tf.placeholder(tf.bool, name='bn_training')
        self.bn_trainable = True
    ...
    def _conv(self, dim, input, filters, kernel_size, padding='same', name=None, activator=None, bn=False, dropout=False):
        ...
        if bn == False:
            ...
            if dropout == True:
                output = tf.layers.dropout(output, rate=self.dropout_rate)
            if activator == None:
                output = tf.layers.batch_normalization(output, training=self.bn_training, momentum=self.momentum, trainable=self.bn_trainable)
            else:
                activator = tf.nn.sigmoid if activator=='sigmoid' else tf.nn.relu
                output = activator(tf.layers.batch_normalization(output, training=self.bn_training, momentum=self.momentum, trainable=self.bn_trainable))
            # output = tf.layers.batch_normalization(output, training=self.bn_training, momentum=self.momentum, trainable=self.bn_trainable)
        return output
    ...
    def _encoderlayer(self, x, idx):
        ...
        with tf.variable_scope('encoderlayer'):
            ...
            state_s = tf.layers.batch_normalization(_x_s + state_s_, name='ec_res{}s'.format(idx), training=self.bn_training, momentum=self.momentum, trainable=self.bn_trainable)
            state_t = tf.layers.batch_normalization(_x_t + state_t_, name='ec_res{}t'.format(idx), training=self.bn_training, momentum=self.momentum, trainable=self.bn_trainable)
            # state_s = _x_s + state_s_
            # state_t = _x_t + state_t_
    ...
    def _decoderlayer(self, y, state, idx):
        ...
        with tf.variable_scope('decoderlayer'):
            ...
            y_s = tf.layers.batch_normalization(_y_s + y_s_, name='dc_res{}s'.format(idx), training=self.bn_training, momentum=self.momentum, trainable=self.bn_trainable)
            y_t = tf.layers.batch_normalization(_y_t + y_t_, name='dc_res{}t'.format(idx), training=self.bn_training, momentum=self.momentum, trainable=self.bn_trainable)
            # y_s = _y_s + y_s_
            # y_t = _y_t + y_t_
            print('decoderlayer')
        return y_s, y_t
    ...
    def _spatialAttn(self, x_s_q, x_s_k, x_s_v, reuse=True):
        with tf.variable_scope('S-Attn', reuse=reuse):
            ...
            o = tf.reshape(o, shape=[-1, f.shape[1], f.shape[2], f.shape[3]])
            # x = gama * o + x
    ...
    def build_model(self):
        ...
        with tf.variable_scope('ST-Attn'):
            ...
            # each station
            y_ = tf.reshape(y_, [-1, self.input_row*self.input_col, y_.shape[-1]])
            w_d = tf.get_variable(shape=[self.input_row*self.input_col, y_.shape[-1], self.output_steps * self.input_channel],
                                  dtype=tf.float32, initializer=self.weight_initializer, name='w-d')
            b_d = tf.get_variable(shape=[self.input_row*self.input_col, self.output_steps * self.input_channel], dtype=tf.float32,
                                  initializer=self.const_initializer, name='b-d')
            yd = []
            for oi in range(self.output_steps*self.input_channel):
                _yd = tf.nn.relu(tf.reduce_sum(tf.multiply(y_, w_d[:,:,oi]), axis=-1) + b_d[:,oi])
                yd.append(_yd)
            y_ = tf.stack(yd, axis=-1)
            y_ = tf.reshape(y_, [-1, self.input_row, self.input_col, y_.shape[-1]])
