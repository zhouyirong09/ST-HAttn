...
class ST_Attn(object):
    ...
    def _encoder(self, x_s):
        with tf.variable_scope('encoder'):
            ...
            x_s = self._conv(3, x_s, self.s_attn_dim, kernel_size=[3, 3, 3], activator='relu')
    ...
    def _decoder(self, state):
        with tf.variable_scope('decoder'):
            ...
            y_s = self._conv(3, y, self.s_attn_dim, [3,3,3], activator='relu', name='Additional-S')
            # change for later shape
            y_ = y_s
            for idx in range(self.NUM_LAYERS):
    ...
    def _spatialAttn(self, x_s_q, x_s_k, x_s_v, reuse=True):
        with tf.variable_scope('S-Attn', reuse=reuse):
            # conv-transform
            f = self._conv(3, x_s_q, self.s_attn_dim,
                           kernel_size=[3, 3, 3], padding='same', name='f',
                           activator='relu', bn=True
                           )
            g = self._conv(3, x_s_k, self.s_attn_dim,
                           kernel_size=[3, 3, 3], padding='same', name='g',
                           activator='relu', bn=True
                           )
            h = self._conv(3, x_s_v, self.s_attn_dim,
                           kernel_size=[3, 3, 3], padding='same', name='h',
                           activator='relu', bn=True
                           )

            # f, g, h
            s = tf.matmul(self._dhw_flatten(f), self._dhw_flatten(g), transpose_b=True) / tf.math.sqrt(self.s_attn_dim*1.0)

            # softmax(Q, K)*V
            beta = tf.nn.softmax(s, axis=-1)
            o = tf.matmul(beta, self._dhw_flatten(h))
            # gama = tf.get_variable('gama', [1], initializer=tf.constant_initializer(0.0))

            o = tf.reshape(o, shape=[-1, h.shape[1], h.shape[2], h.shape[3], h.shape[4]])
            # x = gama * o + x
    ...
    def _positionwisefeedforward(self, x_s, idx):
        with tf.variable_scope('S-FF{}'.format(idx)):
            x_s = self._conv(3, x_s, self.s_attn_dim, kernel_size=[1,1,1], dropout=True, activator='relu', bn=True)
        return x_s#, x_t

    def _hw_flatten(self, x):
        return tf.reshape(x, shape=[-1, x.shape[1]*x.shape[2]*x.shape[3], x.shape[-1]])
    def _dhw_flatten(self, x):
        return tf.reshape(x, shape=[-1, x.shape[1]*x.shape[2]*x.shape[3], x.shape[-1]])
    ...
    def build_model(self):
        ...
        with tf.variable_scope('ST-Attn'):
            ...
            y_ = y_s
            y_ = self._conv(3, y_, filters=2, kernel_size=[3,3,3], activator='relu')
